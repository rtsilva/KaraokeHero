...
def init_midi(filename):
    ...
    x = []
    y = []
    line_one, = ax.plot(x, y, 'bs', markersize=20)
    line_two, = ax.plot([], [], 'ro', markersize =20)
    ...
    return fig, x_data, y_data, line_one, line_two
...
def animate_midi(fig, x_data, y_data, line_one,line_two):
    '''
    input:
    ...
    def init():
        line_one.set_data([i for i in x_range], [0 for i in range(x_r*2)])
        line_two.set_data([10], [0])
    ...
    def animate(i):
        ...
        line_one.set_data(new_x, new_y)

        line_two.set_data([10], [midi%12])
